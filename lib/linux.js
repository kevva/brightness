...
const setBrightness = (device, val) => fsP.writeFile(path.join(dir, device, 'brightness'), val).catch(err => {
	if (err.code === 'EACCES') {
...
module.exports.get = () => {
	...
	return getBacklight()
		...
		.then(res => parseInt(res[1], 10) / parseInt(res[0], 10));
};
